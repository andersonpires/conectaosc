...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($IdInscrito > 0) {
        try {
            ...
            header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/editar/?IdInscrito=$IdInscrito&msg=Dados atualizados com sucesso");
            exit;
        } catch (Throwable $e) {
            header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/editar/?IdInscrito=$IdInscrito&erro=" . urlencode("Erro ao atualizar: " . $e->getMessage()));
            exit;
        }
    }

    header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/?erro=" . urlencode("ID do inscrito não fornecido"));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['IdInscrito'])) {
    ...
    try {
        ...
        if (!$inscrito) {
            header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/?erro=" . urlencode("Inscrito não encontrado"));
            exit;
        }
    } catch (Throwable $e) {
        header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/?erro=" . urlencode("Erro ao buscar inscrito: " . $e->getMessage()));
        exit;
    }
} else {
    header("Location: " . rtrim((string)$BASE_para_URL, "/") . "/eventos/inscritos/?erro=" . urlencode("ID do inscrito não fornecido"));
    ...
    <?php require_once $BASE_para_PATH . '/app/views/partials/menu.php'; ?>
        ...
        <div class="main">
            ...
            <div class="container mt-5">
                ...
                <?php endif; ?>
                <form action="<?php echo rtrim((string)$BASE_para_URL, "/"); ?>/eventos/inscritos/editar/" method="POST">
