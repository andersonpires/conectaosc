...
class VersatilisService
{
    ...
    public static function sincronizarAlunoComVersatilis(array $aluno)
    {
        ...
        if ($cpf === '') {
            return [
                'status' => 'ERRO',
                'mensagem' => 'CPF inválido para sincronizar com Versatilis.',
            ];
        }
        ...
        if ($consulta['status'] === 200) {
            return [
                'status' => 'JA_EXISTE',
                'mensagem' => 'Usuário já cadastrado no Versatilis.',
        ...
        if ($consulta['status'] !== 404) {
            return [
                'status' => 'ERRO',
                'mensagem' => 'Erro ao consultar usuário no Versatilis.',
            ];
        }
        ...
        if ($cadastro['status'] >= 200 && $cadastro['status'] < 300) {
            return [
                'status' => 'CADASTRADO',
                'mensagem' => 'Usuário cadastrado no Versatilis.',
            ];
        }

        return [
            'status' => 'ERRO',
            'mensagem' => 'Erro ao cadastrar usuário no Versatilis.',
        ];
    }
    ...
    private static function obterToken(array $config)
    {
        if ($config['username'] === '' || $config['password'] === '') {
            self::registrarLog([
                'evento' => 'token',
                'erro' => 'Usuário ou senha do Versatilis não configurados.',
            ]);
            return null;
        }
