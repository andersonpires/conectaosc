...
$basePath = $_SESSION['BASE_PATH'] ?? dirname(__DIR__);
...
function resolveLogoPathProntuario($logoRaw, $basePath) {
    ...
    $candidates = [
        ...
        $basePath . '/assets/img/logos/' . ltrim(str_replace('\\', '/', $logoRaw), '/'),
        $basePath . '/assets/img/sistema/' . ltrim(str_replace('\\', '/', $logoRaw), '/'),
        $basePath . '/assets/img/fotos/' . ltrim(str_replace('\\', '/', $logoRaw), '/'),
    ];
    foreach ($candidates as $p) {
        if (is_file($p)) return realpath($p) ?: $p;
    }
    $fallbacks = [
        $basePath . '/assets/img/logos/logoimpressao.png',
        $basePath . '/assets/img/logos/logoimpressao.jpg',
        $basePath . '/assets/img/logos/LogoImpressao.jpg',
        $basePath . '/assets/img/logos/LogoImpressao.png',
        $basePath . '/assets/img/sistema/logo.png',
        $basePath . '/assets/img/logos/logoimpressao.PNG',
    ];
...
if ($assinar) {
    ...
    if ($isDev) {
        ...
        if ($baseUrl === '') {
            $baseUrl = ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http') . '://' . $host . '/conectaosc';
        }
    } else {
        $baseUrl = 'https://iteva.com.br/conectaosc';
    }
    ...
    if (file_exists($basePath . '/lib/tcpdf/fonts/licorice.php')) {
