...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Detectar clique com o botão direito nos cards
        document.querySelectorAll('.card').forEach(card => {
            card.addEventListener('dblclick', function(e) {
                ...
                <?php if ($_SESSION['Tipo'] === 'Administrador') : ?>
                    ...
                    // Verifica se o card contém um botão com classe 'btnp selecionado'
                    if (card.querySelector('.btn.btnp.selecionado')) {
                        ...
                        if (partes.length === 5) {
                            ...
                            // Obter dataSelecionada de algum input oculto ou variável JS (ajustar conforme seu sistema)
            ...
            $.ajax({
                ...
                success: function(resposta) {
                    try {
                        ...
                        // Fecha o primeiro modal (de confirmação)
        ...
        $(document).on('click', '.btnp', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // getFaltas - atualiza o número de faltas caso o usuário mude isso
                    $.ajax({
                        url: 'savebanco.php',
                        type: 'POST',
                        data: {
...
<script>
    $(function() {
        $('#obsModal').on('show.bs.modal', function(event) {
            ...
            var modal = $(this);
            // loadOBS - Lê as observações do banco
            $.ajax({
                url: 'savebanco.php',
                type: 'POST',
                data: {
...
<script>
    document.getElementById("SalvaObs").addEventListener("click", function() {
        ...
        // salvaOBS - Salva as observações no banco
        $.ajax({
            url: 'savebanco.php',
            type: 'POST',
            data: {
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Esconde tooltip ao clicar em qualquer botão dentro de um card
