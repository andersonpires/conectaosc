...
if (isset($_POST['IdColaborador']) && !empty($_POST['IdColaborador'])) {
    ...
    if ($colaborador) {
        // Limpa a máscara do CPF e pega os 6 primeiros dígitos
        ...
        if ($update->execute([$senhaHash, $IdColaboradorAlt, $TimeAlterado, $IdColaborador])) {
            header("Location: resetSenha.php?msg=Senha redefinida com sucesso!");
            exit;
        } else {
            header("Location: resetSenha.php?erro=Erro ao atualizar a senha.");
            exit;
        }
    } else {
        header("Location: resetSenha.php?erro=Colaborador não encontrado.");
        exit;
    }
}
?>
...
<script>
    function resetaSenha(id) {
        ...
        form.method = 'POST';
        form.action = 'resetSenha.php'; // Envia para o próprio arquivo
