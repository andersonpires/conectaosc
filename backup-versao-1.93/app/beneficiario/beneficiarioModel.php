...
require_once $_SESSION['BASE_PATH'] . '/temp/apikeyTwilio.php';
...
class BeneficiarioModel
{
    ...
    public static function create($dados)
    {
        ...
        // --------------------------------------------------
        // ENVIO DE SMS VIA TWILIO APÓS O INSERT
        // --------------------------------------------------
        ...
        if (!empty($telefone)) {
            ...
            // Twilio exige número no formato +55DDDNUMERO
    ...
    public static function salvarInteresses($idUsuario, $idsProjetos)
    {
        ...
        // Busca todos os projetos válidos na tabela tbProjeto
        $stmt = $pdo->query("SELECT IdProjeto FROM tbProjeto");
        $todosProjetos = $stmt->fetchAll(PDO::FETCH_COLUMN);

        // Prepara a query com ON DUPLICATE KEY para atualizar caso já exista
    ...
    public static function update($dados)
    {
        ...
        // Obtem as colunas válidas da tabela
        $colunasValidas = [];
        $stmt = $pdo->query("DESCRIBE tbAluno");
        while ($linha = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $colunasValidas[] = $linha['Field'];
        ...
        // Normalização dos campos de moeda (ANTES DO FOREACH)
        if (isset($dados['RendaMensal'])) {
            $dados['RendaMensal'] = self::limparMoeda($dados['RendaMensal']);
        }
        if (isset($dados['RendaFamiliar'])) {
        ...
        if (empty($camposAtualizar)) {
            // nada para atualizar
            return true; // ou false, você decide
        }
