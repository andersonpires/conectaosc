...
require_once $basePath . '/temp/api-cpf.php';
...
if (strlen($cpf) !== 11) {
    ...
    echo json_encode(['code' => 400, 'message' => 'CPF invalido.']);
    exit;
}

if (empty($apiKeyCPF)) {
    http_response_code(500);
    echo json_encode(['code' => 500, 'message' => 'API key CPF nao configurada.']);
    exit;
}

if (!function_exists('curl_init')) {
    http_response_code(500);
    echo json_encode(['code' => 500, 'message' => 'Extensao cURL indisponivel no servidor.']);
    exit;
}

// Teste rÃ¡pido de saÃ­da (TCP 443) para detectar bloqueio de rede
$sockErrNo = 0;
$sockErrStr = '';
$sock = @fsockopen('apicpf.com', 443, $sockErrNo, $sockErrStr, 5);
if (!$sock) {
    logConsultaCpf("Bloqueio de saÃ­da TCP: {$sockErrStr} ({$sockErrNo})", $logPath);
    http_response_code(502);
    echo json_encode(['code' => 502, 'message' => 'Bloqueio de saÃ­da no servidor (porta 443).']);
    exit;
}
fclose($sock);
