...
class RelatorioRepository
{
    ...
    {
        $sql = "SELECT IdTurma, NomeTurma FROM tbTurma WHERE IdCurso = ?";
        if ($somenteAtivos === 1) {
            $sql .= " AND Habilitado = 1";
        }
        $sql .= " ORDER BY NomeTurma";
    ...
    {
        $sql = "SELECT IdTurma, NomeTurma FROM tbTurma";
        $params = [];
        if ($idCurso !== null) {
            $sql .= " WHERE IdCurso = ?";
            $params[] = $idCurso;
    ...
    {
        $sql = "SELECT a.Nome, a.Nascimento, a.Endereco, a.Bairro, a.Cidade, a.UF,
    ...
    public function listFrequenciaMensal(
        ...
        $sql = "SELECT a.IdUsuario AS IdAluno, a.Nome AS Aluno, c.Dia, c.Mes, c.Ano, c.Data,
