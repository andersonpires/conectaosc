...
final class TurmaService
{
    ...
    public function create(array $payload): int
    {
        ...
        foreach ($nomes as $nome) {
            ...
            $rows[] = [
                'NomeTurma' => $nomeLimpo,
                'IdCurso' => $idCurso,
                'Municipio' => $this->nullableString($payload['Municipio'] ?? null),
                'Local' => $this->nullableString($payload['Local'] ?? null),
                'Obs' => $this->nullableString($payload['Obs'] ?? null),
    ...
    public function update(int $idTurma, array $payload): bool
    {
        return $this->repository->update($idTurma, [
            ...
            'Municipio' => $this->nullableString($payload['municipio'] ?? $payload['Municipio'] ?? null),
            'Local' => $this->nullableString($payload['local'] ?? $payload['Local'] ?? null),
            'Habilitado' => $this->normalizeHabilitado($payload['habilitado'] ?? $payload['Habilitado'] ?? null),
            'Obs' => $this->nullableString($payload['obs'] ?? $payload['Obs'] ?? null),
            'MaxMatriculas' => $this->nullableInt($payload['max-matriculas'] ?? $payload['MaxMatriculas'] ?? null),
        ]);
    }
