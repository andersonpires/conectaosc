...
function turma_foto($pdo, $data, $dataSelecionada, $NNomeCurso, $NNomeTurma)
{
    ...
    $stmt->execute([$_GET['NNomeCurso'], $_GET['NNomeTurma'], $dataSelecionada]);
    ...
    if (count($rows) > 0) {
        foreach ($rows as $row) {
            ...
            $foto = $_SESSION['BASE_para_URL'] . "/assets/img/fotos/" . $row['Foto'];

            $output .= "<div class='card' 
                            id='card_{$IdMatricula}_{$IdTurma}_{$IdCurso}_{$IdAluno}'
                            data-bs-toggle='tooltip' 
                            data-bs-placement='top' 
                            title='Matrícula: {$IdMatricula} | Código aluno: {$IdAluno} | Curso: {$IdCurso} | Turma: {$IdTurma}' 
                            style='background-color: {$corCard};'>";
            $output .= "<img src='$foto' alt='Foto_de_$Nome'>";
            $link = "listTotalChamada.php?idAluno={$IdAluno}&dataSelecionada={$dataSelecionada}&NNomeCurso=" . urlencode($NNomeCurso) . "&NNomeTurma=" . urlencode($NNomeTurma);
