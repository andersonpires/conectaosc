...
require_once $BASE_PATH . '/api/repositories/ConfigRepository.php';
require_once $BASE_PATH . '/api/services/ConfigService.php';
...
<head>
    ...
    <script>
        tinymce.init({
            ...
            images_upload_handler: (blobInfo, progress) => new Promise((resolve, reject) => {
                ...
                xhr.open('POST', '<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/');
                ...
                xhr.onload = () => {
                    ...
                    if (!json || typeof json.location !== 'string') {
                        reject('Resposta inv?f?????T?f?????,????f???,?s?f??s?,?lida: ' + xhr.responseText);
                        return;
                    }
                ...
                xhr.onerror = () => {
                    reject('Erro de conex?f?????T?f?????,????f???,?s?f??s?,?o. C?f?????T?f?????,????f???,?s?f??s?,?digo: ' + xhr.status);
                };
    ...
    <script>
        function mostrarDiv(area) {
            ...
            // Mostra a div espec?f?????T?f?????,????f???,?s?f??s?,?fica
            ...
            if (target) {
                ...
                console.warn('Div n?f?????T?f?????,????f???,?s?f??s?,?o encontrada:', id);
            }
        }
        ...
        function carregarTurmas() {
            ...
            if (idCurso == 0) {
                ...
                fetch('<?php echo $BASE_URL; ?>/get/getTurmasNome.php?IdCurso=all')
                    .then(res => res.text())
                    .then(html => {
            ...
            // Curso espec?f?????T?f?????,????f???,?s?f??s?,?fico
            // Para curso espec?f?????T?f?????,????f???,?s?f??s?,?fico
            fetch('<?php echo $BASE_URL; ?>/get/getTurmasNome.php?IdCurso=' + idCurso)
                .then(res => res.text())
                .then(html => {
                    turmaSelect.disabled = false;
...
<body>
        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid mt-4">
                    ...
                    <!-- Bot?f?????T?f?????,????f???,?s?f??s?,?es de A?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o -->
                    <div class="mb-3 d-flex gap-2">
                        ...
                        <button class="btn btn-secondary btn-sm" onclick="mostrarDiv('timeline')">
                            <i class="fa-solid fa-clock-rotate-left"></i> Ver Hist?f?????T?f?????,????f???,?s?f??s?,?rico
                        </button>
                        <button class="btn btn-warning btn-sm" onclick="mostrarDiv('notificacoes')">
                            <i class="fa-solid fa-bell"></i> Notifica?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es
                        </button>
                    </div>

                    <!-- ?f?????T?f?????,????f???,?s?f??s?,?rea de Tarefas -->
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <div>
                                        ...
                                        <select id="filtroNotificado" class="form-select form-select-sm d-inline-block w-auto">
                                            <option value="0" selected>N?f?????T?f?????,????f???,?s?f??s?,?o notificadas</option>
                                ...
                                <table class="table table-striped table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Respons?f?????T?f?????,????f???,?s?f??s?,?vel</th>
                                            <th>Data/Hora</th>
                                            <th>Status</th>
                                            <th>A?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es</th>
                                        </tr>
                                    </thead>
                                    <tbody id="tabelaTarefasBody">
                                        <!-- Linhas de tarefas ser?f?????T?f?????,????f???,?s?f??s?,?o inseridas aqui via PHP ou JS -->
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    ...
                    <div id="divBusca" style="display: none;">
                        <div class="card shadow-lg">
                            ...
                            <div class="card-body">
                                <!-- Formul?f?????T?f?????,????f???,?s?f??s?,?rio de busca e exibi?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o dos cards aqui -->
                                <form id="formBusca" class="mb-4">
                                    <div class="row mb-4">
                                        ...
                                        <div class="col-md-2">
                                            <label class="form-label" for="dias">Nos ?f?????T?f?????,????f???,?s?f??s?,?ltimos (dias)</label>
                                ...
                                <!-- Aqui ser?f?????T?f?????,????f???,?s?f??s?,?o carregados os cards -->
                                <div class="row" id="cardsAlunos">
                                    <!-- Cards ser?f?????T?f?????,????f???,?s?f??s?,?o inseridos via JS -->
                                </div>
                    ...
                    <!-- ?f?????T?f?????,????f???,?s?f??s?,?rea de Timeline (Notas e Hist?f?????T?f?????,????f???,?s?f??s?,?rico de Intera?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o) -->
                    <div id="divTimeline" style="display: none;">
                        <div class="card shadow-lg">
                            <div class="card-header bg-secondary text-white">
                                Hist?f?????T?f?????,????f???,?s?f??s?,?rico de Intera?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es
                            </div>
                    ...
                    <!-- ?f?????T?f?????,????f???,?s?f??s?,?rea de Notifica?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es -->
                    <div id="divNotificacoes" style="display: none;">
                        <div class="card shadow-lg">
                            <div class="card-header bg-warning text-dark">
                                Notifica?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es
                            </div>
                            <div class="card-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="nota_email">
                                                E-mail: <span id="emailUsuario"></span> <a href="<?php echo $BASE_URL; ?>/perfil?m=mail">(mudar)</a>
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="nota_whatsapp" <?= $desabilita ?>>
                                            <label class="form-check-label" for="nota_whatsapp">
                                                WhatsApp: <span id="zapUsuario"></span> <a href="<?php echo $BASE_URL; ?>/perfil?m=ws" <?php echo ($ativaZap == false) ? 'disabled' : ''; ?>>(mudar)</a>
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check">
                                            ...
                                            <label class="form-check-label" for="tarefa_email">
                                                E-mail: <span id="emailUsuario2"></span> <a href="<?php echo $BASE_URL; ?>/perfil?m=mail">(mudar)</a>
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" id="tarefa_whatsapp" <?= $desabilita ?>>
                                            <label class="form-check-label" for="tarefa_whatsapp">
                                                WhatsApp: <span id="zapUsuario2"></span> <a href="<?php echo $BASE_URL; ?>/perfil?m=ws" <?php echo ($ativaZap == false) ? 'disabled' : ''; ?>>(mudar)</a>
                                            </label>
                                        </div>
                                    </div>
    ...
    <div id="contextMenu" class="custom-context-menu" style="display: none; position: absolute; z-index: 9999;">
        <ul class="list-group list-group-flush mb-0">
            <li class="list-group-item d-flex align-items-center" id="menuVerMatricula" style="cursor:pointer;">
                <i data-feather="book-open" class="me-2"></i> Ver Matr?f?????T?f?????,????f???,?s?f??s?,?cula
            </li>
    ...
    <script>
        function mostrarHistorico(idUsuario) {
            ...
            fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/', {
                    method: 'POST',
                    headers: {
                    ...
                    if (data.status === 'ok') {
                        ...
                        fichaDiv.innerHTML = `
                                                <div class="d-flex align-items-center mb-3">
                                                    <img src="${a.Foto ? '<?php echo $BASE_URL; ?>/assets/img/fotos/' + a.Foto : '<?php echo $BASE_URL; ?>/assets/img/fotos/padrao.jpg'}" class="rounded-circle me-3" width="80" height="80">
                                                    <div>
                                                        <h5 class="mb-0">
                                                            <a href="<?php echo $BASE_URL; ?>/chamada/faltas/?idAluno=${idUsuario}&NNomeCurso=${a.IdCurso}&NNomeTurma=${a.IdTurma}" target="_blank" class="text-decoration-none">
                                                        ...
                                                        <small><strong>Contatos: </strong>
                                                            ${a.Telefone ? `${a.Telefone} (s?f?????T?f?????,????f???,?s?f??s?,? telefone)` : ''}
                                                            ${a.Telefone && a.WhatsApp ? ' | ' : ''}
                                                            ${a.WhatsApp ? `${a.WhatsApp} (telefone e WhatsApp)` : ''}
                                                        </small><br>

                                                        <small><strong>Endere?f?????T?f?????,????f???,?s?f??s?,?o: </strong>${a.Endereco}, ${a.Bairro}, ${a.Cidade}-${a.UF}</small><br>
            ...
            fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/', {
                ...
                .then(data => {
                    if (data.status === 'ok') {
                        ...
                        let html = '<h6 class="mt-4">Hist?f?????T?f?????,????f???,?s?f??s?,?rico de Notas</h6>';
            ...
            fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/', {
                ...
                .then(data => {
                    if (data.status === 'ok') {
                        ...
                        container.innerHTML = `<h6 class="mt-4">Hist?f?????T?f?????,????f???,?s?f??s?,?rico de Notas</h6>`;
    ...
    <script>
            ...
            if (urlParams.has('idUsuario')) {
                ...
                fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/', {
                    ...
                    .then(data => {
                        ...
                        console.error('Erro na requisi?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o:', error);
                    });

            } else {
                mostrarDiv('tarefas'); // padr?f?????T?f?????,????f???,?s?f??s?,?o
            }
            ...
            fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/crm/salvar/', {
                ...
                .then(data => {
                    ...
                    if (data.status === 'ok' && data.alunos.length > 0) {
                        data.alunos.forEach(aluno => {
                            ...
                            card.innerHTML = `
                                                <div class="d-flex shadow-lg rounded mb-3 p-2 bg-light align-items-start justify-content-between card-hover">
                                                    <img src="<?php echo $BASE_URL; ?>/assets/img/fotos/${aluno.Foto || 'padrao.jpg'}" class="rounded-circle me-3" width="50" height="50" alt="Foto">
                            ...
                            // Guardando informa?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es
                        ...
                        container.innerHTML = '<div class="alert alert-warning">Nenhum aluno encontrado com os crit?f?????T?f?????,????f???,?s?f??s?,?rios informados.</div>';
                        document.getElementById('resultadoBuscaCount').textContent = 'Nenhum aluno encontrado com os crit?f?????T?f?????,????f???,?s?f??s?,?rios informados.';
                    }
                });
        }
        ...
        function salvarPreferencias(mostrarMensagem = false) {
            ...
            if (mostrarMensagem) {
                msgEl.innerHTML = `<div class="spinner-border spinner-border-sm text-primary me-2" role="status"></div> Salvando prefer?f?????T?f?????,????f???,?s?f??s?,?ncias...`;
            }
                ...
                .then(data => {
                    if (data.status === 'ok' && mostrarMensagem) {
                        ...
                        const intervalo = setInterval(() => {
                            msgEl.textContent = `Prefer?f?????T?f?????,????f???,?s?f??s?,?ncias salvas com sucesso! ${contador}`;
                            contador--;
                            if (contador < 0) {
    ...
    <script>
        ...
        // Clique nas op?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?es do menu
        document.getElementById('menuVerMatricula').addEventListener('click', function() {
            const url = '<?php echo $BASE_URL; ?>/matriculas/turma/?turma=' + alunoSelecionadoMenu.IdTurma;
            window.open(url, '_blank');
        });

        document.getElementById('menuListarFaltas').addEventListener('click', function() {
            const url = '<?php echo $BASE_URL; ?>/chamada/faltas/?idAluno=' + alunoSelecionadoMenu.IdUsuario + '&NNomeCurso=' + alunoSelecionadoMenu.IdCurso + '&NNomeTurma=' + alunoSelecionadoMenu.IdTurma;
            window.open(url, '_blank');
        });

        document.getElementById('menuDadosAluno').addEventListener('click', function() {
            abrirPostNovaAba('<?php echo $BASE_URL; ?>/beneficiarios/cadastro', {
                IdUsuario: alunoSelecionadoMenu.IdUsuario
            });
        });

        // Fun?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o para abrir via POST
