        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid p-0">
                    ...
                    // Se n?f?????T?f??s?,?o tiver CPF, mostra apenas o campo para digit?-lo
                    if (!$cpfInformado) {
                    ?>
                        <form method="GET" action="<?php echo rtrim((string)$BASE_URL, '/'); ?>/beneficiarios/cadastro/">
                            <div class="card shadow p-4 col-md-6 mx-auto mt-5">
                                <h3 class="mb-3">Digite o CPF para continuar</h3>
                                <div class="mb-3">
                                    <input type="text" class="form-control" name="cpf" id="cpfInput" maxlength="11" placeholder="Somente n?meros" required autofocus>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Inicializa?f?????T?f??s?,??f?????T?f??s?,?o da M?f?????T?f??s?,?scara
        cpfInput.mask('000.000.000-00', {
            reverse: true
        });

        // Fun??o de atualiza?f?????T?f??s?,??f?????T?f??s?,?o de status
        function updateValidationStatus(isValid) {
            ...
            if (isValid === null) {
                ...
                statusDiv.html('<span style="color: red;">CPF inv?f?????T?f??s?,?lido.</span>');
                submitButton.prop('disabled', false);
            } else {
                statusDiv.html('<span style="color: orange;">Erro na valida?f?????T?f??s?,??f?????T?f??s?,?o.</span>');
                submitButton.prop('disabled', false);
            }
        }

        // L?gica de Valida?f?????T?f??s?,??f?????T?f??s?,?o AJAX
            ...
            if (rawCpf.length === 11) {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        value: rawCpf, // Voltamos a usar a vari?f?????T?f??s?,?vel digitada, em vez do fixo
                        type: 'cpf'
                    },
...
<form action="<?php echo rtrim((string)$BASE_URL, '/'); ?>/beneficiarios/cadastro/" id="formulario" enctype="multipart/form-data" method="post">
    ...
    <?php if ($modoEdicao): ?>
        ...
        <input type="hidden" name="IdColaboradorAlt" value="<?= $_SESSION['Cod'] ?>">
        <input type="hidden" name="TimeAlterado" value="<?= date('Y-m-d H:i:s') ?>">

    <?php else: ?>
        <input type="hidden" name="IdColaboradorEnt" value="<?= $_SESSION['Cod'] ?>">
    ...
    <ul class="nav nav-pills gap-2 mb-4" id="benefTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link tab-inscricao <?= $tabAtiva === 'inscricao' ? 'active' : '' ?>" id="tab-inscricao-btn" data-bs-toggle="tab" data-bs-target="#tab-inscricao" type="button" role="tab" aria-controls="tab-inscricao" aria-selected="<?= $tabAtiva === 'inscricao' ? 'true' : 'false' ?>" data-tab="inscricao">
                Dados de Inscri?f?????T?f??s?,??f?????T?f??s?,?o
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link tab-socio <?= $tabAtiva === 'socio' ? 'active' : '' ?>" id="tab-socio-btn" data-bs-toggle="tab" data-bs-target="#tab-socio" type="button" role="tab" aria-controls="tab-socio" aria-selected="<?= $tabAtiva === 'socio' ? 'true' : 'false' ?>" data-tab="socio">
                Dados Econ?micos e Socioassistenciais
            </button>
        </li>
        <li class="nav-item" role="presentation">
    ...
    <div class="tab-content" id="benefTabsContent">
        <div class="tab-pane fade <?= $tabAtiva === 'inscricao' ? 'show active' : '' ?>" id="tab-inscricao" role="tabpanel" aria-labelledby="tab-inscricao-btn">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <h1 class="h3 mb-0 section-title">DADOS DE INSCRI??O</h1>
            </div>
    <div class="col-11">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-end mb-3">
                    <?php if (!empty($_POST['IdUsuario'])): ?>
                        <a class="btn btn-success" target="_blank" href="<?= $BASE_URL ?>/get/getPDF_ficha_cadastro.php?id=<?= $_POST['IdUsuario'] ?>">
                ...
                <div class="row mb-3">
                    <div class="mb-3 col-8">
                        ...
                        <input type="hidden" name="Colaborador" value="<?php echo $_SESSION['Cod'] ?>">
                    </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-7">
                        <label for="Endereco" class="form-label">Endere?f?????T?f??s?,?o*</label>
                        <input type="text" class="form-control" id="Endereco" name="Endereco" required value="<?= $_POST['Endereco'] ?? '' ?>" autocomplete="off">
                    </div>
                    <div class="col-3">
                        <label for="Numero" class="form-label">N?f?????T?f??s?,?mero</label>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-2">
                        ...
                        <select class="form-select" aria-label="Selecione o Estado" id="UF" name="UF">
                            ...
                            $estadoSelecionado = $_POST['UF'] ?? 'CE'; // padr?o para CE se n?f?????T?f??s?,?o vier nada
                ...
                <div class="mb-3">
                    <?php
                    $fotoAtual = $_POST['Foto'] ?? 'padrao.jfif';
                    $caminhoFoto = $BASE_URL . "/assets/img/fotos/" . $fotoAtual;
                    ?>
                    <br>
                    <img src="<?= htmlspecialchars($caminhoFoto) ?>" class="rounded-circle img-cover" width="100px" height="100px" id="currentPhoto">
                    <br>
                    <label for="uploadImage" class="form-label">Foto benefici?f?????T?f??s?,?rio</label>
                ...
                ?>
                    ...
                    <div class="text-center mb3">
                        <p class="text-center">SE ? INSCRI??O, SELECIONE OS INTERESSES:</p>
                    </div>
                    <div class="checkbox-container">
                        ...
                        if (isset($_POST['IdUsuario'])) {
                            // Pega os projetos que o usu?f?????T?f??s?,?rio j?f?????T?f??s?,? selecionou
                ...
                <?php
                    ...
                    echo "<br><br>";
                    echo '<p class="text-center">Nenhum interesse dispon?vel ou erro ao acessar o banco de dados.</p>';
                }
                ?>
        ...
        <div class="tab-pane fade <?= $tabAtiva === 'socio' ? 'show active' : '' ?>" id="tab-socio" role="tabpanel" aria-labelledby="tab-socio-btn">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <h1 class="h3 mb-0 section-title">DADOS ECON?f?????T?f??s?,?MICOS E SOCIOASSISTENCIAIS</h1>
            </div>
    <div class="col-11">
        <div class="card shadow-lg">
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">N?f?????T?f??s?,? de pessoas na resid?ncia</label>
                        <input type="number" name="NumPessoasReside" class="form-control" value="<?= $_POST['NumPessoasReside'] ?? '' ?>">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Total de c?modos em sua casa?</label>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="ComQuemMora" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Sozinho", "Fam?lia nuclear", "Parentes", "Outros"];
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label">Situa?f?????T?f??s?,??f?????T?f??s?,?o</label>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="PCDEmCasa" class="form-select">
                            ...
                            <option value="0" <?= ($_POST['PCDEmCasa'] ?? '') == '0' ? 'selected' : '' ?>>N?f?????T?f??s?,?o</option>
                        </select>
                    </div>

                    <!-- Quantos e quais defici?ncias -->
                    <div class="col-md-5">
                        <label class="form-label">Quantos e quais defici?ncias</label>
                        <input type="text"
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Situa?f?????T?f??s?,??f?????T?f??s?,?o de moradia</label>
                        <select name="SituacaoMoradia" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Pr?pria", "Alugada", "Cedida", "Ocupa?f?????T?f??s?,??f?????T?f??s?,?o / Invas?f?????T?f??s?,?o", "Outros"];
                    ...
                    <div class="col-md-4">
                        <label class="form-label">Tipo da constru??o</label>
                    ...
                    <div class="col-md-4">
                        <label class="form-label">Abastecimento de ?gua</label>
                        <select name="AbastecimentoAgua" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Rede p?f?????T?f??s?,?blica", "Po?o", "Outro"];
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Esgotamento sanit?rio</label>
                        <select name="EsgotamentoSanitario" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Rede p?f?????T?f??s?,?blica", "Fossa s?f?????T?f??s?,?ptica", "C?u aberto"];
                    ...
                    <div class="col-md-4">
                        <label class="form-label">Rede el?trica?</label>
                        <select name="PossuiRedeEletrica" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["1" => "Sim", "0" => "N?f?????T?f??s?,?o"];
                    ...
                    <div class="col-md-4">
                        <label class="form-label">Ilumina?f?????T?f??s?,??f?????T?f??s?,?o p?f?????T?f??s?,?blica?</label>
                ...
                <div class="mb-3">
                    <label class="form-label">Avalia?f?????T?f??s?,??f?????T?f??s?,?o de Vulnerabilidade Social (IA)</label>
                    <textarea
        ...
        <div class="tab-pane fade <?= $tabAtiva === 'medico' ? 'show active' : '' ?>" id="tab-medico" role="tabpanel" aria-labelledby="tab-medico-btn">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <h1 class="h3 mb-0 section-title">HIST?RICO M?DICO</h1>
            </div>
    <div class="col-11">
        <div class="card shadow-lg">
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Est? em acompanhamento m?dico?</label>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Uso cont?nuo de medicamentos?f?????T?f??s?,?</label>
                ...
                <div class="mb-3">
                    <label class="form-label">Limita?f?????T?f??s?,??f?????T?f??s?,?es f?sicas / locomo??o</label>
                    <input type="text" name="LimitacoesLocomocao" class="form-control" value="<?= $_POST['LimitacoesLocomocao'] ?? '' ?>">
                </div>
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">J? fez fisioterapia?</label>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Pratica atividade f?sica?</label>
                    ...
                    <div class="col-md-8">
                        <label class="form-label">Qual / Frequ?ncia</label>
                        <input type="text" name="TipoFrequenciaAtividade" class="form-control" value="<?= $_POST['TipoFrequenciaAtividade'] ?? '' ?>">
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Recomenda?f?????T?f??s?,??f?????T?f??s?,?o m?dica para evitar esfor?o f?sico?</label>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Usa bebidas alco?licas?f?????T?f??s?,?</label>
                ...
                <div class="mb-3">
                    <label class="form-label">Hist?rico familiar de doen?as</label>
                    <textarea name="HistoricoFamiliarDoencas" class="form-control"><?= $_POST['HistoricoFamiliarDoencas'] ?? '' ?></textarea>
                </div>
                <div class="row mb-3">
                    <div class="col-md-4">
                        <label class="form-label">Alergia a medicamentos?f?????T?f??s?,?</label>
                ...
                <div class="mb-3">
                    <label class="form-label">Observa?f?????T?f??s?,??f?????T?f??s?,?es adicionais de sa?de</label>
        ...
        <div class="card shadow-lg">
            <div class="card-body">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Em caso de emerg?ncia, a quem costuma ligar?</label>
                        <input type="text" name="ContatoEmergencia" class="form-control" value="<?= $_POST['ContatoEmergencia'] ?? '' ?>">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label">Qual sua cor ou ra?a?</label>
                        <select name="CorRaca" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Branco", "Pardo", "Preto", "Amarelo", "Outra", "Prefiro n?f?????T?f??s?,?o responder"];
                            $valor = $_POST['CorRaca'] ?? '';
                            foreach ($opcoes as $op) {
                                $selected = ($valor == $op) ? 'selected' : '';
                                echo "<option value=\"$op\" $selected>$op</option>";
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Qual import?ncia voc? atribui na participa?f?????T?f??s?,??f?????T?f??s?,?o da sua fam?lia/amigos no curso?</label>
                        <select name="ImportanciaFamiliaAmigos" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Irrelevante", "Pouco", "Muito", "Prefiro n?f?????T?f??s?,?o responder"];
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label">Quem ? a pessoa com quem voc? tem maior v?nculo familiar?</label>
                        <input type="text" name="VinculoFamiliar" class="form-control" value="<?= $_POST['VinculoFamiliar'] ?? '' ?>">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Voc? costuma receber visitas em casa?</label>
                        <select name="RecebeVisitas" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Sim", "N?f?????T?f??s?,?o", "Raramente", "Sempre"];
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label">At? que ponto sua audi??o ou vis?f?????T?f??s?,?o prejudicam sua capacidade de interagir no projeto?</label>
                        <select name="ImpactoAudicaoVisao" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Nada", "Pouco", "M?dio", "Muito"];
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Como voc? avalia sua escrita?</label>
                        <select name="AvaliacaoEscrita" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["N?f?????T?f??s?,?o sei escrever", "Baixa", "M?dia", "Boa", "?tima"];
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <label class="form-label">Como voc? avalia sua leitura?</label>
                        <select name="AvaliacaoLeitura" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["N?f?????T?f??s?,?o sei ler", "Baixa", "M?dia", "Boa", "?tima"];
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Como voc? avalia seu condicionamento f?sico?</label>
                        <select name="CondicionamentoFisico" class="form-select">
                            <option value="">Selecione</option>
                            <?php
                            $opcoes = ["Baixo", "M?dio", "Bom", "?timo"];
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <label class="form-label">Atualmente voc? frequenta algum outro curso? Se sim, qual?</label>
                        <input type="text" name="OutroCursoAtual" class="form-control" value="<?= $_POST['OutroCursoAtual'] ?? '' ?>">
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="col-12">
                        <label class="form-label">Voc? tem interesse em algum curso espec?fico que ainda n?f?????T?f??s?,?o fez, mas gostaria de fazer? Se sim, qual?</label>
    ...
    <div class="col-11">
        <div class="card shadow-lg">
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-6">
                        ...
                        <select class="form-select" aria-label="Selecione sua Escolaridade" id="Escolaridade" name="Escolaridade">
                            ...
                            $escolaridadeSelecionada = $_POST['Escolaridade'] ?? 'Fundamental incompleto'; // valor padr?o
                    ...
                    <div class="col-6">
                        ...
                        <select class="form-select" aria-label="Selecione seu estado civil" id="EstadoCivil" name="EstadoCivil">
                            ...
                            $opcoesEstadoCivil = [
                                ...
                                "Separado",
                                "Vi?vo",
                                "Uni?o est?f?????T?f??s?,?vel",
                                "Outro"
                            ];
                            $estadoCivilSelecionado = $_POST['EstadoCivil'] ?? 'Solteiro'; // valor padr?o
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <label for="Profissao" class="form-label">Profiss?f?????T?f??s?,?o</label>
                        <input type="text" class="form-control" id="Profissao" name="Profissao" value="<?= $_POST['Profissao'] ?? '' ?>" autocomplete="off">
                    </div>
                </div>
                <br>
                <h5>RESPONS?f?????T?f??s?,?VEIS</h5>
                <hr class="close-hr"><br>
                <div class="mb-3">
                    <label for="NomeResp1" class="form-label">Nome do respons?f?????T?f??s?,?vel 1</label>
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <label for="NomeResp2" class="form-label">Nome do respons?f?????T?f??s?,?vel 2</label>
                ...
                <hr class="close-hr"><br>
                <div class="mb-3">
                    <label for="Obs" class="form-label">Observa?f?????T?f??s?,??f?????T?f??s?,?o</label>
                    <textarea rows="5" class="form-control" aria-label="Observa?f?????T?f??s?,??f?????T?f??s?,?es" id="Obs" name="Obs" autocomplete="off"><?= $_POST['Obs'] ?? '' ?></textarea>
                </div>
                <div class="d-flex justify-content-end gap-2 flex-wrap">
                    <button type="submit" name="acao" value="salvar" class="btn btn-outline-secondary" data-acao="fechar">
                        Salvar e fechar
                    ...
                    <button type="submit" name="acao" value="salvar_versatilis" class="btn" style="background-color: #03989E; color: #ffffff;" <?= $podeUsarVersatilis ? '' : 'disabled' ?> title="<?= $podeUsarVersatilis ? '' : 'Sem permissao Versatilis' ?>">
                        Salvar + Versatilis
                    </button>
                </div>
            </div>
...
<!-- Importa?f?????T?f??s?,??f?????T?f??s?,?o do Jquery Mask -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.14.12/jquery.mask.min.js"></script>
<script>
    ...
    uploadImage.addEventListener('change', function(event) {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            alert("Por favor, envie uma imagem v?f?????T?f??s?,?lida.");
            uploadImage.value = '';
        }
    });
</script>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $.ajax({
            url: '<?php echo rtrim((string)$BASE_URL, '/'); ?>/beneficiarios/consulta-cpf/',
            type: 'GET',
            dataType: 'json',
            data: {
                cpf: cpf
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function atualizarClassificacaoSocioeconomica() {
            ...
            // Troca v?rgula por ponto (caso o usu?f?????T?f??s?,?rio digite assim)
            ...
            // Se estiver faltando dado ou inv?f?????T?f??s?,?lido, limpa o campo
        ...
        // Dispara o c?lculo sempre que mudar RendaFamiliar ou NumPessoasReside
        $('input[name="RendaFamiliar"], input[name="NumPessoasReside"]').on('input change', function() {
            atualizarClassificacaoSocioeconomica();
        });

        // Se j?f?????T?f??s?,? vier valor do banco (modo edi??o), calcula na carga da p?f?????T?f??s?,?gina
        atualizarClassificacaoSocioeconomica();
    });
</script>
...
<script>
    (function() {
        ...
        function garantirAcaoNoPost(valor) {
            // garante que acao v? no POST mesmo com submit program?tico
        ...
        form.addEventListener('submit', function handler(e) {
            const btn = e.submitter; // bot?o clicado (quando existe)
            ...
            if (acao === 'avaliar_vulnerabilidade') {
                ...
                // Remove qualquer overlay para n?f?????T?f??s?,?o bloquear a visualiza?f?????T?f??s?,??f?????T?f??s?,?o da textarea
                ...
                const statusMensagens = [
                    ...
                    { text: '? Buscando dados do banco para compara?f?????T?f??s?,??f?????T?f??s?,?o', duration: 5000 },
                    { text: '? Fazendo an?lise de vulnerabilidades', duration: 5000 },
                ...
                fetch('<?php echo rtrim((string)$BASE_URL, '/'); ?>/beneficiarios/avaliar-vulnerabilidade-stream/', {
                    method: 'POST',
                    body: formData,
                    ...
                    if (!response.ok || !response.body || !contentType.includes('text/event-stream')) {
                        ...
                        throw new Error(texto || 'Falha ao iniciar a avalia?f?????T?f??s?,??f?????T?f??s?,?o.');
                    }
                    ...
                    const processarChunk = ({ value, done }) => {
                        ...
                        partes.forEach(parte => {
                            ...
                            if (evento === 'delta') {
                                ...
                                // Ignorado: os status s?f?????T?f??s?,?o controlados no cliente
                    ...
                    if (typeof mostrarOverlay === 'function') {
                        mostrarOverlay({
                            mensagem: err.message || 'Erro ao gerar avalia?f?????T?f??s?,??f?????T?f??s?,?o.',
                            carregando: false
                        });
                    } else {
                        alert(err.message || 'Erro ao gerar avalia?f?????T?f??s?,??f?????T?f??s?,?o.');
                    }
                }).finally(() => {
                    submitted = false;
                });
