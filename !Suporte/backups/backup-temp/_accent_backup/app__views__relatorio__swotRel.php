...
require_once $BASE_PATH . '/app/swot/swotModel.php';

$temas = [
    'INOVA?f?????T?f?????,????f??,??f???,?s?,??f??s?,??f?????T?f?????,????f???,???f????s????z?O EM PROJETOS EXISTENTES',
    'NOVOS PROJETOS PARA INVESTIDORES',
    'NOVOS SERVI?f?????T?f?????,????f??,??f???,?s?,??f??s?,?OS',
    'NOVOS PRODUTOS'
];
...
if ($filtrar) {
    ...
    if (!empty($dados)) {
        foreach ($dados as $item) {
            ...
            if ($nomeCompleto === '') {
                $nomeCompleto = 'N?f?????T?f?????,????f???,?s?f??s?,?o informado';
            }
        ...
        <div class="main">
            ...
            <main class="content">
                <div class="container-fluid p-0">
                    <h1 class="h3 mb-3">Relat?rio SWOT</h1>
                                ...
                                <div class="row g-3 align-items-end">
                                    ...
                                    <div class="col-md-3 d-flex gap-2">
                                        ...
                                        <?php if ($filtrar && !empty($dados)): ?>
                                            <?php
                                            $pdfUrl = $BASE_URL . '/get/getSwotPDF.php?tema=' . urlencode($temaSelecionado) . '&usuario=' . urlencode($usuarioSelecionado);
                                            ?>
