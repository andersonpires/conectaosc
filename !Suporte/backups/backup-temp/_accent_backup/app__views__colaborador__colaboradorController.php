...
$COLABORADORES_ROUTE = rtrim((string)$BASE_URL, '/') . '/colaboradores';
...
require_once __DIR__ . '/colaboradorModel.php';
require_once __DIR__ . '/especialidadeProfissionalModel.php';
...
function tratarUploadFoto($arquivo, $fotoAtual = null)
{
    ...
    $caminhoArquivo = $BASE_PATH . "/assets/img/fotos/" . $nomeArquivo;

    if (move_uploaded_file($arquivo['tmp_name'], $caminhoArquivo)) {
        if ($fotoAtual && $fotoAtual !== 'padrao.jfif') {
            $caminhoAntigo = $BASE_PATH . "/assets/img/fotos/" . $fotoAtual;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($acao === 'excluir') {
        ...
        if ($id > 0 && ColaboradorModel::inativar($id, $idColaboradorAlt, $timeAlterado)) {
            header("Location: {$COLABORADORES_ROUTE}/?msg=" . urlencode("Usu?f?????T?f?????,????f???,?s?f??s?,?rio inativado com sucesso."));
            exit;
        }

        header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Erro ao inativar usu?f?????T?f?????,????f???,?s?f??s?,?rio."));
        exit;
    }
    ...
    if ($acao === 'reativar') {
        ...
        if ($id > 0 && ColaboradorModel::reativar($id, $idColaboradorAlt, $timeAlterado)) {
            header("Location: {$COLABORADORES_ROUTE}/?status=inativos&msg=" . urlencode("Usu?f?????T?f?????,????f???,?s?f??s?,?rio reativado com sucesso."));
            exit;
        }

        header("Location: {$COLABORADORES_ROUTE}/?status=inativos&erro=" . urlencode("Erro ao reativar usu?f?????T?f?????,????f???,?s?f??s?,?rio."));
        exit;
    }
    ...
    if ($acao === 'salvar') {
        ...
        if ($nome === '' || $sobrenome === '' || $email === '' || $idPermissao <= 0) {
            header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Preencha os campos obrigat?f?????T?f?????,????f???,?s?f??s?,?rios."));
            exit;
        }

        $tipo = ColaboradorModel::getTipoPermissao($idPermissao);
        if (!$tipo) {
            header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Permiss?f?????T?f?????,????f???,?s?f??s?,?o inv?f?????T?f?????,????f???,?s?f??s?,?lida."));
            exit;
        }
        ...
        if ($id > 0) {
            ...
            if (!$registro) {
                header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Usu?f?????T?f?????,????f???,?s?f??s?,?rio n?f?????T?f?????,????f???,?s?f??s?,?o encontrado para edi?f?????T?f?????,????f???,?s?f??s?,??f?????T?f?????,????f???,?s?f??s?,?o."));
                exit;
            }
            ...
            if (ColaboradorModel::update($dados)) {
                if (!EspecialidadeProfissionalModel::replaceForColaborador($id, $especialidadesInput)) {
                    header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Registro salvo, mas houve erro ao salvar especialidades."));
                    exit;
                }
                $mensagem = "
                    <!DOCTYPE html>
                ...
                header("Location: {$COLABORADORES_ROUTE}/?msg=" . urlencode("Registro alterado com sucesso."));
                exit;
            }

            header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("N?f?????T?f?????,????f???,?s?f??s?,?o foi poss?f?????T?f?????,????f???,?s?f??s?,?vel alterar o registro."));
            exit;
        }

        if ($cpf === '' || $senha === '') {
            header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("CPF e senha s?f?????T?f?????,????f???,?s?f??s?,?o obrigat?f?????T?f?????,????f???,?s?f??s?,?rios."));
            exit;
        }
        ...
        if ($novoId) {
            ...
            $url = $baseUrl !== '' ? $baseUrl . "/login/?confirma=" . urlencode($confirma) : "login/?confirma=" . urlencode($confirma);
            $logoSrc = $baseUrl !== '' ? $baseUrl . "/assets/img/sistema/logo.png" : "/assets/img/sistema/logo.png";
            ...
            $mensagem = "
                ...
                <body>
                    <div class='header'>
                        <img src='{$logoSrc}' id=ITEVALogo alt=ITEVA class='align-middle' width='110px'>
                    </div>
                    <div class='content'>
                        <p class='chamada'>Ola, {$nome}.</p>
            ...
            enviarEmail($nome, $email, $mensagem);
            header("Location: {$COLABORADORES_ROUTE}/?msg=" . urlencode("Colaborador cadastrado com sucesso."));
            exit;
        }

        header("Location: {$COLABORADORES_ROUTE}/?erro=" . urlencode("Erro ao cadastrar registro."));
        exit;
    }
}
...
try {
    ...
    // tb_especialidade pode n?f?????T?f?????,????f???,?s?f??s?,?o existir ainda
}
...
require __DIR__ . '/formColaborador.php';
exit;
