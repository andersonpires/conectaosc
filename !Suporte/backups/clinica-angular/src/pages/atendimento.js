...
export async function renderAtendimento(container, opts = {}) {
  ...
  const [consultasParaAtendimento, pacientesRes] = await Promise.all([
    getConsultasParaAtendimento(dataConsulta).catch(() => []),
    getPacientes(),
  ]);
  const { pacientes } = pacientesRes;

  const consultaSelecionada = preselectConsultaId
    ? consultasParaAtendimento.find((c) => c.id === preselectConsultaId)
    : null;
  ...
  ` + (consultasParaAtendimento.length === 0
    ...
    <div class="space-y-3 mb-6">
      ...
      <div class="grid gap-3 sm:grid-cols-2">
        ${consultasParaAtendimento.map((c) => {
  ...
  const formSection = `
    <div id="atend-form-section" class="bg-white rounded-xl shadow-monday p-4 space-y-4 ${!consultaSelecionada ? 'opacity-60 pointer-events-none' : ''}">
      ...
      <div>
        ...
        <select id="atend-paciente" class="w-full px-4 py-3 border border-gray-300 rounded-lg" ${consultaSelecionada ? '' : 'disabled'}>
          <option value="">Selecione...</option>
          ${pacientes.map((p) => `<option value="${p.IdUsuario}" ${consultaSelecionada?.aluno_id == p.IdUsuario ? 'selected' : ''}>${escapeHtml(String(p.Nome || ''))}</option>`).join('')}
        </select>
      </div>
      ...
      <div id="tab-prontuario" class="tab-content hidden">
        <div>
          ...
          <p class="text-xs text-gray-500 mb-1">O prontuário é gerado com base no cadastro (tbAluno) e na anamnese. Use este campo para complementar.</p>
          <textarea id="atend-dados" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg" placeholder="Queixa, achados ou notas do atendimento..." ${consultaSelecionada ? '' : 'disabled'}></textarea>
        </div>
        ...
        <div id="resultado-ia" class="hidden">
          ...
          <input type="hidden" id="atend-consulta-id" value="${consultaSelecionada?.id || 0}">
  ...
  container.querySelectorAll('.atend-consulta-card').forEach((btn) => {
    btn.onclick = () => {
      ...
      renderAtendimento(container, { consultaId: id });
    };
  });

  const consultaIdInput = container.querySelector('#atend-consulta-id');
  const selectPaciente = container.querySelector('#atend-paciente');
  ...
  if (consultaSelecionada) {
    ...
    if (consultaSelecionada.aluno_id) selectPaciente.value = String(consultaSelecionada.aluno_id);
  }

  selectPaciente?.addEventListener('change', async () => {
    const pid = parseInt(selectPaciente.value, 10);
  ...
  btnSalvarAnamnese?.addEventListener('click', async () => {
    const consultaId = consultaSelecionada?.id;
    const alunoId = parseInt(selectPaciente.value, 10);
    if (!consultaId || !alunoId) {
      alert('Selecione uma consulta e um paciente.');
      return;
    }
  ...
  container.querySelector('#btn-salvar-pront').onclick = async () => {
    ...
    if (!alunoId || !conteudo) {
      alert('Selecione paciente e gere o prontuário com IA.');
      return;
    }
